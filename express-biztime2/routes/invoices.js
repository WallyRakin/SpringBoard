...
router.route('/')
    .get(async (req, res, next) => {
        try {
            const invoices = await db.query('SELECT id, comp_code FROM invoices').then(response => { return response.rows });
...
router.route('/:id')
    ...
    .delete(async (req, res, next) => {
        try {
            const { id } = req.params;
            const invoice = await db.query('DELETE FROM invoices WHERE id = $1 RETURNING *', [id]).then(response => { return response.rows[0] });
